...
type EstimateFormProps = {
  // Resolved server-side from the ?product= search param (set by product-page
  // CTAs) so the prefill is present in the initial render — the page itself
  // decides whether that query param also makes the URL noindex.
  initialProductInterest?: string;
};

export default function EstimateForm({ initialProductInterest = "" }: EstimateFormProps) {
  const [submitted, setSubmitted] = useState(false);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [files, setFiles] = useState<File[]>([]);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [productInterest] = useState(initialProductInterest);
